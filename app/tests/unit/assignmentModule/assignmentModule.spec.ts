...
describe("Assignment Module - container", () => {
  ...
  it("Assignment module container should display assignment name", () => {
    ...
    expect(
      assignmentmodulename
        ...
        .toLowerCase()
        .trim()
    ).to.equal(dummyAssignmentModule.AssignmentName.toLowerCase().trim());
  });
  ...
  it("Assignment module container should display a date for when the assignment is due", () => {
    ...
    expect(
      assignmentmoduledate
        .text()
        .toLowerCase()
        .trim()
    ).to.equal("due " + dummyAssignmentModule.Date.toLowerCase().trim());
  });
});
...
describe("Assignment Module - Reading Section", () => {
  ...
  it("Assignment Module reading section should display each reading assignment", () => {
    ...
    expect(readingassignments.length).to.equal(
      dummyAssignmentModule.ReadingList.length
    );
    readingassignments.forEach(
      (assingment: DOMWrapper<Element>, index: number) => {
        expect(
          assingment
            .text()
            .toLowerCase()
          ...
          dummyAssignmentModule.ReadingList[
            index
          ].ReadingDesc.toLowerCase().trim()
        );
      }
    );
  });
...
describe("Assignment Module - Tests Section", () => {
  ...
  it("Assignment Module reading section should display each test assignment", () => {
    ...
    expect(testassignments.length).to.equal(
      dummyAssignmentModule.TestList.length
    );
    testassignments.forEach(
      (assingment: DOMWrapper<Element>, index: number) => {
        expect(
          assingment
            .text()
          ...
          dummyAssignmentModule.TestList[index].TestDesc.toLowerCase().trim()
        );
      }
    );
  });
