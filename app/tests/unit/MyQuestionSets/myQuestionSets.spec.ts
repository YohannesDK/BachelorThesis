...
(async () => {
  ...
  describe("MyQuestionSet - table", () => {
    ...
    it("MyQuestionSets table row should av a dropdown button for displaying actions regarding that questionset", () => {
      expect(MyQuestionSetTableRowDropDownbtn.length).to.equal(
    ...
    it(`MyQuestionSets dropdown should be visible and have these field: ${dropdownoptions}, when button is pressed`, () => {
      MyQuestionSetTableRowDropDownbtn.forEach((btn: DOMWrapper<Element>) => {
        ...
        wrapper.vm.$nextTick(() => {
          ...
          const dropdownitems = MyQuestionSetTableRowDropDown.findAll(
            "[data-test-id='myquestionset-options-dropdown-items']"
          );
          const dropdownresult = dropdownoptions.every(
            (dropdownoption: string) => {
              return (
                dropdownitems
                  .map((item: DOMWrapper<Element>) => item.text().toLowerCase())
                  .indexOf(dropdownoption) >= 0
              );
            }
          );
