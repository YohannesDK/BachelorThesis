...
describe("Dashboard - welcome message", () => {
  it("welcome message should exist", () => {
    const welcomeMessage = wrapper.find(
      "[data-test-id='dashboard-welcome-message']"
    );
...
describe("Dashboard - recents card", () => {
  ...
  it("recents card should have a navbar", () => {
    const recentsCardNavBar = wrapper.find(
      "[data-test-id='dashboard-recents-card-navbar']"
    );
    expect(recentsCardNavBar.exists()).to.equal(true);
  });

  const recentsCardNavBarOptions = wrapper.findAll(
    "[data-test-id='dashboard-recents-card-navbar-option']"
  );
  it("recents card navbar should have 4 nav-items", () => {
    ...
    expect(recentsCardNavBarOptions[0].text().toLowerCase()).to.equal(
      "documents"
    );
  });
  ...
  it("recents card navbar should have courses as the second nav item", () => {
    expect(recentsCardNavBarOptions[1].text().toLowerCase()).to.equal(
      "courses"
    );
  });

  it("recents card navbar should have questions as the third nav item", () => {
    expect(recentsCardNavBarOptions[2].text().toLowerCase()).to.equal(
      "questions"
    );
  });

  it("recents card navbar should have resources as the fourth nav item", () => {
    expect(recentsCardNavBarOptions[3].text().toLowerCase()).to.equal(
      "resources"
    );
  });
});
...
describe("Dashboard - notifications card", () => {
  it("notifications card should exists", () => {
    const notificationcard = wrapper.find(
      "[data-test-id='dashboard-notification-card']"
    );
