  ...
  await wrapper.vm.$nextTick(() => {
    describe("TakeTest - navbar", () => {
      it("Take Test navbar should exists", () => {
        const TakeTestNavbar = wrapper.find(
          "[data-test-id='TakeTest-test-navbar']"
        );
        expect(TakeTestNavbar.exists()).to.equal(true);
      });

      const TakeTestNavbarTittle = wrapper.find(
        "[data-test-id='TakeTest-test-navbar-tittle']"
      );
      ...
      it(`Take Test navbar tittle should be: ${dummyQuestionSet.Tittle}`, () => {
        expect(TakeTestNavbarTittle.text().toLowerCase()).to.equal(
          dummyQuestionSet.Tittle.toLowerCase()
        );
      });
    });

    describe("TakeTest - container", () => {
      const TakeTestContainer = wrapper.find(
        "[data-test-id='TakeTest-test-container']"
      );
      ...
      it(`Take Test container should contain same number of question card as questions in questionset: 
        ...
        const TakeTestQuestions = TakeTestContainer.findAll(
          "[data-test-id='TakeTest-test-questions']"
        );
        expect(TakeTestQuestions.length).to.equal(
          dummyQuestionSet.QuestionSet.length
        );
      });
    });
    ...
    describe("TakeTest - question navigation sidebar", () => {
      const TakeTestNavigationSideBar = wrapper.find(
        "[data-test-id='TakeTest-test-question-navigation-sidebar']"
      );
      const TakeTestNavigationSideBarItem = TakeTestNavigationSideBar.findAll(
        "[data-test-id='TakeTest-test-question-navigation-sidebar-item']"
      );
      ...
      it(`Take Test SideBar should have same number of items as there are questions: ${dummyQuestionSet.QuestionSet.length}`, () => {
        expect(TakeTestNavigationSideBarItem.length).to.equal(
          dummyQuestionSet.QuestionSet.length
        );
      });
      ...
      it(`Take Test Sidebar should enable user to focus on selected question set when clicking on the button`, () => {
        ...
        TakeTestNavigationSideBarItem.forEach(
          (btn: DOMWrapper<Element>, index: number) => {
            btn.trigger("click");
            wrapper.vm.focusIndex === index
              ? result.push(true)
              : result.push(false);
          }
        );
    ...
    describe("TakeTest - submit button", () => {
      it("Take Test should have a submit button, for submitting test when done!", () => {
        const finishbtn = wrapper.find(
          "[data-test-id='TakeTest-test-btn-submit']"
        );
