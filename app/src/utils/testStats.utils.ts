...
export function SingeTestStatResult(QSID: number, TestQuestionAndAnswer: TestQuestionAndAnswer) : number {
  ...
  if (qs) {
    qs.QuestionSet.find((question: Question) => {
      if (question.QuestionID === TestQuestionAndAnswer.QuestionID) {
        if (question.QuestionType === QuestionTypeEnum.ShortText || question.QuestionType === QuestionTypeEnum.LongText) {
          result = (
            (
              question.Question.Answer as string).trim().toLowerCase() === (TestQuestionAndAnswer.Answer as string).trim().toLowerCase()
              ?
        ...
        if (question.QuestionType === QuestionTypeEnum.TrueFalse || question.QuestionType === QuestionTypeEnum.MultipleChoice) {
          if (question.Question.CorrectAnswer !== undefined) {
            result =  (
              question.Question.CorrectAnswer === (TestQuestionAndAnswer.Answer as number)
              ?
              TestStatAnswerCorrect.CORRECT
              :
              TestStatAnswerCorrect.WRONG
